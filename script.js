...
let pendingFolders = []; // [{ id, name, files: File[] }]
let currentFolderPathInModal = null;
let currentFolderDisplayNameInModal = null;
let currentGalleryImages = [];
let appInitialized = false;

const pendingFolderListEl = document.getElementById("pendingFolderList");
...
// ==== Chọn folder (có thể chọn nhiều lần để thêm vào hàng đợi) ====
folderInput.addEventListener("change", (e) => {
  handleFileList(e.target.files);
  folderInput.value = ""; // cho phép chọn lại folder khác (hoặc cùng folder) lần nữa
});
...
function readDroppedItems(items) {
  return new Promise((resolve) => {
    ...
    // Hỗ trợ kéo-thả NHIỀU folder cùng lúc: mỗi item đầu vào có thể là 1 folder gốc riêng
...
// Nhận danh sách file (từ input hoặc kéo-thả), gom nhóm theo folder gốc,
// rồi thêm từng nhóm vào hàng đợi pendingFolders (không xoá các folder đã có sẵn trong hàng đợi)
function handleFileList(fileList) {
  ...
  files.forEach(f => {
    ...
    const topFolder = relPath.includes("/") ? relPath.split("/")[0] : `Folder_${Date.now()}`;
...
function renderPendingFolders() {
  ...
  pendingFolders.forEach(pf => {
    ...
    item.className = "pending-folder-item";
    item.innerHTML = `
      <span class="pf-icon">📂</span>
      <span class="pf-name">${escapeHtml(pf.name)}</span>
      <span class="pf-count">${pf.files.length} ảnh</span>
      <button type="button" class="pf-remove" data-id="${pf.id}">✕ Bỏ</button>
    `;
    pendingFolderListEl.appendChild(item);
  });

  pendingFolderListEl.querySelectorAll(".pf-remove").forEach(btn => {
    btn.addEventListener("click", () => {
      pendingFolders = pendingFolders.filter(pf => pf.id !== btn.dataset.id);
      renderPendingFolders();
    });
  });
...
// ==== Lưu tất cả folder trong hàng đợi lên Supabase Storage ====
saveBtn.addEventListener("click", async () => {
  ...
  const totalFiles = pendingFolders.reduce((sum, pf) => sum + pf.files.length, 0);

  let doneFiles = 0;
  let failed = 0;

  for (const pf of pendingFolders) {
    // Gắn hậu tố __exp<timestamp> (tự xoá sau RETENTION_DAYS) hoặc __perm<timestamp> (giữ vĩnh viễn)
    const suffix = isPermanent ? `__perm${Date.now()}` : `__exp${Date.now()}`;
    const folderPath = `${sanitizeName(pf.name)}${suffix}`;
    ...
    for (const file of pf.files) {
      ...
      progressText.textContent = `${doneFiles}/${totalFiles} ảnh · ${folderCount} folder`;
    }
  }
  ...
  alert(failed
    ? `Đã lưu xong ${folderCount} folder nhưng có ${failed} ảnh lỗi (xem console để biết chi tiết).`
    : `✅ Đã lưu thành công ${folderCount} folder!`);

  renderFolderList();
});

// Đọc thông tin hạn từ tên folder trên storage.
// __exp<timestamp>  -> folder có hạn, tự xoá sau RETENTION_DAYS
// __perm<timestamp> -> folder giữ vĩnh viễn, không bao giờ tự xoá
// __<timestamp>      -> định dạng cũ (trước khi có tính năng giữ vĩnh viễn), coi như có hạn
function parseFolderMeta(folderName) {
  let m = folderName.match(/__exp(\d+)$/);
  if (m) return { type: "exp", ts: parseInt(m[1], 10) };

  m = folderName.match(/__perm(\d+)$/);
  if (m) return { type: "perm", ts: parseInt(m[1], 10) };

  m = folderName.match(/__(\d+)$/);
  if (m) return { type: "exp", ts: parseInt(m[1], 10) };

  return null;
}

function stripFolderSuffix(folderName) {
  return folderName.replace(/__(exp|perm)?\d+$/, "");
}

// ==== Tự động xoá folder cũ hơn RETENTION_DAYS (bỏ qua folder giữ vĩnh viễn) ====
async function cleanupExpiredFolders(folders) {
  ...
  for (const folder of folders) {
    ...
    if (meta && meta.type === "exp" && (now - meta.ts) > RETENTION_MS) {
      try {
...
function getExpiryInfo(folderName) {
  ...
  const expiresAt = meta.ts + RETENTION_MS;
  const msLeft = expiresAt - Date.now();
  const daysLeft = Math.max(0, Math.ceil(msLeft / (24 * 60 * 60 * 1000)));
  return { permanent: false, daysLeft, msLeft };
}
...
async function renderFolderList() {
  ...
  for (const folder of folders) {
    ...
    const displayName = stripFolderSuffix(folder.name);
    ...
    card.innerHTML = `
      ...
      <div class="fmeta">
        ...
        ${expiry
          ? (expiry.permanent
              ? `<span class="expiry-badge permanent">♾ Vĩnh viễn</span>`
              : `<span class="expiry-badge ${expiry.daysLeft <= 1 ? 'soon' : ''}">Còn ${expiry.daysLeft} ngày</span>`)
          : ""}
      </div>
